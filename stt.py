...
class STT:
    ...
    def post_process_whisperx(self, transcription_segments, audio_segment):
        ...
        try:
            ...
            logger.debug(f"실제 음성 세그먼트 {len(aligned_segments)}")
            logger.debug("==========whisperX=========")
            logger.debug(aligned_segments)
            return aligned_segments
        except Exception as e:
            logger.debug(f"후처리_에러: {e}")
            return None
    ...
    def run(self, input_audio_url, num_speakers):
        ...
        if json_content is None:
            logger.debug("json_content_생성_실패")
            return
